...
func mountRoutes(rootCtx context.Context, app *fiber.App, cfg *config.Config, gdb *gorm.DB, objectStore storage.Storage) {
	...
	siswaGroup.Get("/bantuan-admin/chat", chatHandler.GetSiswaAdminChat)
	siswaGroup.Post("/bantuan-admin/chat/messages", chatHandler.SendSiswaAdminMessage)
	siswaGroup.Post("/bantuan-admin/chat/read", chatHandler.MarkSiswaAdminRead)
	...
	cleanupOverride := strings.EqualFold(os.Getenv("ENABLE_TESTING_CLEANUP"), "true")
	ujianHandler.EnableTestingCleanup(!cfg.IsProduction() || cleanupOverride)
