...
| 2026-05-25 | Apis | Batch 3 soal latihan | PASS | QA passwords normalized to `[REDACTED]`; guru/siswa login OK; existing `Kelas QA Aktif` + `Bab QA Materi dan Latihan` reused; 5 soal mode `keduanya` ready; invalid bulk paste returns 400 `rows_required`; siswa starts latihan, answers 5 soal, immediate feedback includes `is_benar` + `jawaban_benar`; finish returns summary benar=5/total=5; retry after finish creates new attempt. |
| 2026-05-25 | Apis | Batch 4 ulangan bab | PASS | Guru upserts ulangan setting for `Bab QA Materi dan Latihan` (`jumlah_soal=5`, `durasi_menit=10`, `batas_attempt=3`); siswa lobby reads configured setting; siswa starts ulangan with 5-question pool; live items strip `jawaban_benar` anti-cheat; answers save via shared answer endpoint; submit is idempotent and returns benar=5/total=5/nilai_total=5; review disabled returns 403 `review_disabled`, then review opens after guru enables setting; guru hasil-rekap works; guru cancel/reset attempt lets siswa start a fresh ulangan. Note: first probe confirmed insert path ignored `izinkan_review_setelah_submit=false`; update path persisted it, worth revisiting if default insert behavior matters. |
| 2026-05-25 | Apis | Batch 5 tugas submission grading | PASS | Guru creates published bab-scoped `Tugas QA Batch 5` with bobot 100; siswa sees tugas via `/siswa/kelas/:id/tugas`; siswa submits text-only multipart; resubmit before grading updates same submission and increments version; siswa own-submission reflects revised catatan; guru list submissions sees row; guru grades with nilai 88 + feedback; siswa sees graded feedback; resubmit after graded returns 409 `already_graded`; guru pending-counts endpoint responds after grading. |
| 2026-05-25 | Apis | Batch 6 bank soal ujian bobot | PASS | Guru creates 5 Bank Soal QA Batch 6 items and filter by mapel/tingkat/topik returns pool; manual published ujian with bobot 50 uses selected bank soal; source preview returns pool; siswa sees manual + random ujian via `/siswa/kelas/:id/ujian`; manual start returns 5 items without answer-key leak; answer save is delayed-grade (`ok=true`); submit returns nilai_total=5 with 5/5 correct and is idempotent; review open works; guru hasil-rekap works; cancel/reset attempt lets siswa start fresh; random ujian snapshots 3 items from filtered bank pool. |
| 2026-05-25 | Apis | Batch 7 rekap nilai guru export CSV | PASS | Guru rekap kelas via `/kelas/:id/rekap` returns matrix for `Kelas QA Aktif`: siswa row present, summary `siswa_count`/`siswa_with_nilai` valid, bab and ujian headers populated, QA student has bab nilai and ujian attempt data, ujian bobot 50/100 visible, `total_kelas` computed. CSV export via `?format=csv` returns `text/csv`, header starts `siswa_id,siswa_nama,total_kelas`, includes `bab_*_total` and `ujian_*_terbaik` columns, and QA student row has total value. Siswa-side `/siswa/kelas/:id/nilai` also returns matching kelas and total. |
| 2026-05-25 | Apis | Batch 8 nilai siswa formula validation | PASS | Siswa nilai kelas via `/siswa/kelas/:id/nilai` returns matching `Kelas QA Aktif`; formula bab validates equal-component average: ulangan bab 0 and tugas 88 produce total/`total_kelas` 44; breakdown weights are equal (`w=1` for ulangan and tugas); tugas metadata shows 1 graded task with bobot 100; ulangan metadata shows 5 soal and hasil id; ujian rows expose bobot plus `nilai_terbaik`/`nilai_terakhir`, attempt count, and hasil id; cross-class `/siswa/nilai` card matches class total and bab/ujian counts. |
| 2026-05-25 | Apis | Batch 9 archive/delete visibility | PASS | Dummy kelas archive endpoint sets `archived_at` and guru list with `include_archived=true` includes it; joining archived invite returns `kelas_archived`. Bab archive hides published bab from `/siswa/kelas/:id/bab`; tugas delete hides it from siswa list and `GET /tugas/:id` returns 404; pengumuman archive hides it from siswa list; pengumuman hard delete returns 404. Initial bug: archived kelas still appeared in `/siswa/kelas` for already-enrolled siswa. Fixed in backend `ListMyKelas` by filtering `ArchivedAt != nil`; deployed and verified `before_total=1`, `after_total=0`. |
| 2026-05-25 | Apis | Batch 10 security guards | PASS | Protected endpoint without token returns `unauthorized`; siswa/guru hitting `/admin/users/` returns `forbidden`; siswa creating kelas returns `forbidden`; cross-guru kelas detail returns `forbidden`; siswa accessing unjoined kelas bab returns `forbidden`; must-change-password user is blocked from `/siswa/kelas` with `must_change_password`, can call change-password, then `/siswa/kelas` succeeds; refresh rotation rejects reuse of old refresh token with `invalid_credentials`. |
| 2026-05-25 | Apis | Batch 11 R2/image storage | PASS | `/readyz` storage check reports `ok (r2:lms-dev)`; Bank Soal image upload rejects non-image multipart with `unsupported_mime`; valid PNG upload stores object key under `soal-bank/...png` and returns dimensions; presigned image URL returns matching object key; replacing the image returns `replaced=true`; delete clears the slot and subsequent image-url returns `image_slot_empty`. |
| 2026-05-25 | Apis | Batch 12 operational checks | PASS | `/healthz` OK, `/readyz` OK with database/storage ready, `lms-api` service active. Manual backup created `lms_manual_2026-05-25_061327.sql.gz` (232482 bytes); `gunzip -t` passed; disposable restore DB `lms_restore_drill_20260525061328` restored successfully with users=22 and kelas=36, then dropped. `cleanup-dry-run --format=json` stayed dry-run with candidate counts 0 for login attempts, refresh tokens, and deleted ujian; `hasil_soal_bab_deleted_old` remains unavailable because `deleted_at` column is absent. Final admin login smoke passed. |
