...
- Roadmap & locked decisions: `.kiro/steering/lms-roadmap.md` (v0.11.13 — Fase 4 ✅ CLOSED 14/14, **Fase 5 ✅ CLOSED 15/15 + UX/QA pass `22d2095`**, **Fase 6 plan ✅ DECOMPOSED 15 task locked #83-#88 + 6.A.1 ✅ DONE `3371e30` + 6.B 3/3 ✅ CLOSED `f50e7f2`+`76de898`+`ceaf86b` + 6.C 2/2 ✅ CLOSED `ede3194` + 6.D 4/4 ✅ CLOSED `7d465bf`+`d2ecef9`+`205be54`+`0df6f89` + 6.E 1/1 ✅ CLOSED `8f77dbc` + 6.F 2/2 ✅ CLOSED `1269846`+`446f187` + 6.G.1 ✅ DONE `d9012b1` — Fase 6 BE 100%, FE Guru 100%, FE Siswa 1/2**).
...
- [ ] Fase 6 — Ulangan Harian (cross-bab) — plan ✅ DECOMPOSED 15 task locked #83-#88; estimasi 7 hari. **6.A foundation 1/1 ✅ DONE** commit `3371e30` — migration 000011 + 7 model + repo skeleton, 6 tables (bank_soal, ujian, ujian_soal, hasil_ujian, jawaban_ujian, event_ujian) + 17 indexes, schema_version `000011_ujian`. **6.B BankSoal 3/3 ✅ CLOSED** — 6.B.1 `f50e7f2` CRUD per-guru pribadi (smoke 26/26) + 6.B.2 `76de898` image upload 6-slot R2 prefix `soal-bank/` + WebP→JPEG fallback + presign inline 15m + atomic-swap old-key cleanup + version orthogonal (smoke 40/40) + 6.B.3 `ceaf86b` bulk paste pipe-delimited 8 kolom + tag default per-batch + 7 reason codes + cap 200 line + escape `\|` literal (smoke 27/27). **6.C Ujian setup 2/2 ✅ CLOSED** — 6.C.1+6.C.2 `ede3194` CRUD/duplicate (junction copied via SetUjianSoalIDs, R2 image keys SHARED via BankSoal pribadi guru locked #84) + source mode dispatch manual/random (locked #85) + PreviewSource + active-attempts guard (smoke 52/52). **6.D Ujian flow 4/4 ✅ CLOSED** — 6.D.1 `7d465bf` Start deterministic seed locked #86 + Items anti-cheat strip (smoke 28/28); 6.D.2 `d2ecef9` answer save delayed grade locked #76 mirror (UPSERT JawabanUjian is_benar=NULL+poin_dapat=0; anti-cheat soal_in_pool guard; smoke 32/32); 6.D.3 `205be54` submit + auto-grade tx + advisory lock locked #87 (single-tx grade pakai banksoal.Jawaban as truth; pg_advisory_xact_lock("hasil-submit:"||id); idempotent already_submitted; late-submit grace 5s 410 submit_after_grace; concurrent-safe winner+loser; smoke 32/32 incl concurrent 2 parallel + DB lifecycle + audit single-event); 6.D.4 `0df6f89` timer cron 30s + auto-grade tx + shared advisory lock locked #87 (NEW `internal/ujian/timer_cron.go` separate goroutine `go ujianTimerCron.Run(rootCtx)`; ScanExpiredHasilIDs + per-row tx + submitLockKey shared with Submit; selesai_at=deadline_at preservation; action=`auto_grade` trigger=`cron`; refactor `gradeAttemptInTx` shared helper; smoke /tmp/qa-6d4.sh 20/20 incl cron-auto-grade + submit-beats-cron-race + cancelled-skip + idempotent + selesai_at=deadline_at). **6.E Hasil consolidated 1/1 ✅ CLOSED** — 6.E.1 `8f77dbc` Review+ListSiswa+Cancel+Rekap (mirror 5.E.1 `8c55651`). 4 endpoints: GET /siswa/hasil-ujian/:id/review (two-sentinel gating review_disabled 403 vs review_locked 403, locked #81), GET /siswa/kelas/:id/ujian/hasil (cross-ujian list per kelas via JOIN ujian.kelas_id, aggregate single-pass), POST /hasil-ujian/:id/cancel (guru/admin RoleGuard flat group, soft-cancel Status='dibatalkan'+DeletedAt release partial-unique slot, idempotent fast-path 200+snapshot, EventUjian+audit prev_status forensics), GET /ujian/:id/hasil-rekap (three-structure aggregate, sort *float64 DESC nulls-last+name ASC, rata-rata over students-with-score, narrow userLookup nil-safe). Pool-snapshot delete-tolerant render. Pattern skill `go-attempt-readside-review-cancel-rekap`. Smoke /tmp/qa-6e1.sh 37/37 PASS. Coverage gate locked #88 ≥70% defer to Fase 8 TODO (mirror Fase 5 #82 soft fallback). **Fase 6 BE 100% (10/10).** **6.F FE Guru 2/2 ✅ CLOSED** — 6.F.1 `1269846` `/guru/bank-soal` top-level page (locked #84 BankSoal pribadi cross-kelas) + `BankSoalList` filter chip Mapel/Tingkat derived dari master query + Topik substring search + pagination 25/page server limit/offset + `BankSoalEditDialog` (tag mapel/tingkat/topik + 5 opsi + jawaban radio + poin + 6 image slots presigned, drop `mode` dropdown vs SoalBab, default tag dari filter aktif) + `BankSoalBulkPasteDialog` 8-col + tag default + partial-success panel + nav entry "Bank Soal" di sidebar. Smoke /tmp/qa-6f1.sh 14/14 PASS. 6.F.2 `446f187` Ulangan tab di kelas detail page + `UjianList` (status/source mode badge + jumlah_soal preview + chevron expand inline rekap + Edit/Duplikat/Hapus) + `UjianFormDialog` (judul/deskripsi/durasi/timing/review-toggle/status/source) + `UjianSourceConfigPanel` discriminated locked #85 (manual filter+checkbox grid; random filter+jumlah_soal; Live Preview button POST /ujian/:id/source/preview verify pool_size; pool warning state) + `RekapHasilUjianTable` per-siswa aggregate (attempt/cancelled/nilai_terbaik/terakhir/status_terakhir + Reset attempt button) + `CancelUjianAttemptDialog` confirm soft-cancel POST /hasil-ujian/:id/cancel (locked #76 dibatalkan tidak count, slot released untuk fresh attempt). frontend/lib/ujian-api.ts typed client + friendlyUjianError. Tab nav guru kelas detail ditambah "Ulangan" (GraduationCap icon). Page size guru/kelas/detail 6.94→16.7 kB, First Load 269 kB, masuk static export. Smoke /tmp/qa-6f2.sh 14/14 PASS. **6.G FE Siswa 1/2** — **6.G.1 `d9012b1`** `/siswa/ujian` top-level cross-kelas lobby (mirror /siswa/tugas pattern) + `UjianLobbyCard` (window state badge mendatang/aktif/berakhir/tersedia + source mode badge + 4-tile grid durasi/jumlah_soal/attempt_count/nilai_terbaik + countdown auto-tick 1s ke waktu_mulai/waktu_selesai + review policy note locked #81 + last attempt line + PrimaryCTA dispatch inflight=Lanjutkan / mendatang=disabled "Mulai dalam ..." / berakhir=disabled / aktif=Mulai/Mulai-baru) + filter chips status + dropdown kelas + sort aktif-deadline-first / mendatang-soonest-first / berakhir-most-recent + counters tile 4-up. CTA navigate ke /siswa/kelas/detail/ujian?id=K&uid=U deliberate two-stage rollout (route 404 sampai 6.G.2 ship). frontend/lib/siswa-ujian-api.ts typed client siswa-side + friendlySiswaUjianError 16-code mapper. **BE FIX:** siswaGroup.Get("/kelas/:id/ujian") di main.go reuse ujianHandler.ListByKelas — service-level role-branch siswa published-only + verify enrollment (sebelumnya endpoint cuma admin/guru, siswa kena 403). Page /siswa/ujian 10.5 kB / First Load 124 kB. Sidebar nav siswa entry "Ujian saya" GraduationCap icon. Smoke /tmp/qa-6g1.sh 18/18 PASS. Sisa Fase 6: 6.G.2 FE Siswa player+review state machine (mirror 5.G.2 `6c10d19`).
