...
- Active focus: **Fase 8 — Polish + E2E / production-readiness**. Error key naming cleanup deferred from 7.F done for guru audit invalid kelas id (`invalid_id`). Playwright E2E skeleton now has 3 login/auth smokes including force-change-password redirect; local `npm run typecheck` + `npx playwright test --list` pass and remote `E2E_BASE_URL=http://127.0.0.1:8200 npm run e2e` passes 3/3 after deploy (`readyz OK`). Backup/restore drill passed on rdpkhorur using manual backup `lms_manual_2026-05-23_033715.sql.gz` restored into disposable DB `lms_restore_drill_20260523033818` (`users=7`, `kelas=28`, drop PASS). Cleanup dry-run passed after restore drill; all candidate counts 0, destructive cleanup remains gated/off. Coverage gate measured and partially improved: `auth` 55.8%, `admin` 75.4%, `soalbab` 7.6% (bulk parser tests), `ujian` 9.0% (source/timestamp/error mapper tests), `nilai` 23.6% (handler tests). Recommendation locked for next step: do **not** block production-awal on strict 70% per-package coverage; re-scope coverage gate for MVP go-live, add representative E2E core flows + final clean deploy smoke, then document known coverage gap as v0.14/v0.15 hardening. **Side-quest done 2026-05-25**: siswa UI theme refactor selesai end-to-end (head `3b6d960`). Admin/guru tetap neutral shadcn.
- Latest side-quest 2026-05-24: grade weighting moved from class-level to item-level and deployed (head `ab7278c`). `tugas.bobot` + `ujian.bobot` migration `000013_item_bobot` applied; old rows default `100`; `bobot=0` allowed to exclude item from weighted average. Tugas per bab formula = `sum(nilai * bobot) / sum(bobot)`; `NilaiBab` now averages Ulangan Bab + Tugas equally and skips NULL, no longer uses `kelas.bobot_soal_ulangan` / `kelas.bobot_tugas`. Class-level bobot fields remain in DB/API response for legacy compatibility only; create/update kelas ignore bobot payload and keep legacy defaults/retained values. UI no longer asks/shows bobot kelas; bobot is configured per tugas/ujian only. Guru class list is now school-aware: `/api/v1/kelas?sekolah_id=...` filters by school, response includes `sekolah_nama`, and `/guru/kelas` cards show school name + school filter dropdown. Latest commits: `3ccd66b`, `06a2ca8`, `b51da72`, `27f0488`, `30c3487`, `e05b554`, `ab7278c`.
- Latest deployed 2026-05-25: pengumuman rich text + multi-attachment shipped/deployed (head `7f2085e`). Backend migration `000015_pengumuman_multi_attachment` creates `pengumuman_attachment` and backfills legacy single attachment. Pengumuman content now uses existing markdown/rich editor; guru/admin + siswa lists render markdown via `react-markdown` + `remark-gfm`. Attachment API supports max 10 files per pengumuman, add multiple files, delete/presign per attachment ID, with legacy `PUT /pengumuman/:id/attachment` kept as alias. FE helper `pengumumanAttachments()` preserves legacy single-attachment fallback. Validation: `go test ./internal/pengumuman`, `go test ./...`, `npm run typecheck` all PASS; deployed to rdpkhorur; `/api/v1/readyz` ready; migration version `15`; `pengumuman_attachment` table exists. Build/lint warnings from older Next areas are deferred to Fase 8 polish/QA, not blocking.
- Latest deployed 2026-05-26: pengumuman compose/edit UX polish deployed (head `082acb0`). `MarkdownEditor` now supports optional preview/toolbar toggles; pengumuman create/edit keep rich text toolbar enabled but hide the live `Pratinjau` pane so `Isi pengumuman` stays full-width. Validation: local `npm run typecheck` PASS; pushed origin+server; deployed via `deploy/deploy.sh --remote`; `/api/v1/readyz` ready. Commit chain: `0abde09` hide preview/full-width input, `082acb0` restore rich editor toolbar.
- Latest deployed 2026-05-26: siswa soal image preview popup + zoom shipped/deployed (head `ef2f27f`). Added reusable `ZoomableSoalImage` in `frontend/components/siswa-ui/`; applied to `LatihanPlayer`, `UlanganPlayer`, and `UjianPlayer` for pertanyaan/opsi images. Inline image remains visible; click opens `.siswa-theme` Dialog preview with zoom in/out/reset and scroll/pan area. Validation: local `npm run typecheck` PASS; pushed origin+server; deployed via `deploy/deploy.sh --remote`; server HEAD `ef2f27f`; `/api/v1/readyz` ready (`database` ok, `storage` ok).
- Latest deployed 2026-05-27: siswa self-registration shipped/deployed (head `e8dde07`). Admin sekolah now has `siswa_registration_enabled` + mode `approval_required`/`auto_approve`; public `/register` lets siswa pick sekolah+kelas from admin-created data; backend adds migration `000016_siswa_self_registration`, public school/class endpoints, `POST /auth/register-siswa`, and admin/guru join request approval endpoints. Deploy PASS via `deploy/deploy.sh --remote`; migration version `000016_siswa_self_registration`; `/api/v1/readyz` ready. Smoke PASS: public sekolah/kelas, register pending mode creates `siswa_join_requests`, auto mode creates active `enrollment`, cleanup verified pending smoke rows 0.
- Latest fix 2026-05-27: `/register` blank white client-side exception fixed/deployed (head `8b5fdfa`). Root cause: public sekolah endpoint returned JSON `data:null` when no school has self-registration enabled, and register page called `.find()` on `null`. Fix: backend public sekolah/kelas always returns empty arrays (`[]`), frontend normalizes `sekolahQ.data?.data ?? []`. Deploy PASS via `deploy/deploy.sh --remote`; Playwright live check `https://lms.fikriramaaulia.online/register` status 200, form visible, console/page errors empty; API `/api/v1/public/sekolah` now returns `{"data":[]}`.
- Latest fix 2026-05-27: kelas creation moved to admin panel and deployed (head `a58b6c9`). Admin now has menu/page `/admin/kelas` with list/filter and `Tambah kelas` dialog: admin selects guru pengampu + sekolah + nama/deskripsi. Guru `/guru/kelas` no longer exposes create button; it only lists classes assigned by admin. Backend `POST /api/v1/kelas` now allows admin payload `guru_id` and validates the assigned user role is `guru`; guru self-create remains API-compatible but hidden from UI. Deploy PASS via `deploy/deploy.sh --remote`; `/api/v1/readyz` ready; server HEAD `a58b6c9`.
- Latest fix 2026-05-27: admin kelas edit/archive actions deployed (head `bab1621`). `/admin/kelas` cards now expose `Edit` (nama/deskripsi using optimistic `version`) and `Arsipkan` confirmation; list invalidates after create/edit/archive. Deploy PASS via `deploy/deploy.sh --remote`; frontend static build includes `/admin/kelas`; backend build PASS; `/api/v1/readyz` ready; server HEAD `bab1621`.
- Latest fix 2026-05-27: `/register` empty state deployed (head `1adf273`). If no school has self-registration enabled, page now shows explicit message `Pendaftaran mandiri belum dibuka`, disables school select + submit, and handles public sekolah load errors. Deploy PASS via `deploy/deploy.sh --remote`; Playwright live check `https://lms.fikriramaaulia.online/register` status 200, empty state visible, no `Application error`, no console/page errors.
- Latest correction 2026-05-27: restored guru class creation and deployed (head `1142ba7`). Product expectation clarified: admin owns `rombel/kelas resmi` master data (VII-A/VII-B) for school structure/self-registration, while guru still creates and fully manages their own learning classes/courses. `/guru/kelas` create dialog/button restored; build/deploy PASS via `deploy/deploy.sh --remote`; `/api/v1/readyz` ready. Recommendation doc `.kiro/steering/lms-admin-kelas-self-registration-recommendations.md` updated to reflect rombel vs guru course split.
- Latest UI update 2026-05-27: moved temporary rombel management into admin Sekolah page and deployed (head `b3506ea`). Admin sidebar label is now `Sekolah & Rombel`; `/admin/kelas` is removed from sidebar. In `/admin/sekolah`, editing a school shows `Rombel <school>` section with add/edit/archive. Underlying temporary implementation still uses `/kelas` table/API with hidden fallback active guru_id until a real `rombel` table is implemented. Lint/build PASS (existing warnings only), remote deploy PASS, `readyz OK`.
- Latest fix 2026-05-28: ulangan bab attempt limit now supports custom 1-999 plus unlimited mode and deployed (head `65a0bad`). Migration `000018_ulangan_unlimited_attempts` adds `attempt_unlimited` and relaxes `batas_attempt` CHECK to 1-999. Backend skips attempt cap when unlimited, returns `attempt_unlimited` in lobby/start payloads, and guru setting API persists it. Guru form has unlimited checkbox; siswa lobby shows Unlimited and does not block start. Validation: local `go test ./...` PASS, local `npm run build` PASS with existing lint warnings only, deploy PASS, remote HEAD `65a0bad`, migration version `18`, `/api/v1/readyz` ready.
...
- [x] Siswa UI theme refactor (neo-brutalism + pastel pop) — head `3b6d960`, deployed to rdpkhorur. Tokens di `app/globals.css` namespaced `.siswa-theme`, palette pastel (yellow/pink/blue/green/lavender/cream) + 2px borders + hard offset shadows + Space Grotesk display font. 8 primitives di `components/siswa-ui/` (SiswaCard/Button/Stat/EmptyState/PageHeader+Highlight/Badge/Progress/Section). Per-section accent: Materi=blue, Latihan=green, Ulangan=yellow, Tugas=pink, Nilai=lavender, Umum=cream. Deterministic kelas tone via `kelasToneFromId(id)`. Layout shell drop max-w cap → full viewport (`bf138b4` fix). Touched 24 file siswa-only across 3 commits: `bf138b4` (theme baseline + dashboard + gabung + kelas detail), `c51de37` (gutter fix), `76ee35b` (kelas/detail/bab + tugas + ujian + nilai + kelas/detail/{nilai,tugas} + 5 shared comps SiswaTugasList/UjianLobbyCard/SiswaNilaiBabTable/SiswaNilaiUjianList/SubmissionPanel), `3b6d960` (PengumumanReadList + MateriViewer/PdfViewer/YouTubeEmbed/MarkdownView + LatihanPlayer + UlanganLobby/Section/Player/Review + UjianPlayer/Review/Section). Admin/guru shells **untouched** — keep neutral shadcn. Locked decisions: skip dark mode, players retain reading-friendly surface tone (timer warn=yellow/critical=rose pop only for wayfinding), submit confirm dialogs stay shadcn modal default.
...
- [ ] Fase 8 — Polish + E2E (active; pengumuman rich text + multi-attachment shipped in head `7f2085e`; next backlog includes frontend warning cleanup and broader E2E/manual smoke)
...
## Siswa UI design tokens (locked v0.13.x)
- Palette pastel: yellow `#FFD93D`, pink `#FF90E8`, blue `#90DBF4`, green `#B5EAD7`, lavender `#CBB7F7`, cream `#FFF6CC`. Dark slate text `#1a1a1a`, near-black border `#0a0a0a`.
- Borders 2px (soft black), shadows hard offset 3-4px (no blur), radius 16-20px. Section accents semantic: Materi=blue, Latihan=green, Ulangan=yellow, Tugas=pink, Nilai=lavender, Umum=cream.
- Fonts: Inter (body) + Space Grotesk (display, 400-700) loaded via `next/font/google` di root layout. Display var only applied di `.siswa-theme` namespace.
- Components: gunakan `@/components/siswa-ui` primitives (SiswaCard, SiswaButton, SiswaBadge, SiswaStat, SiswaEmptyState, SiswaPageHeader, SiswaProgress, kelasToneFromId, SECTION_META) untuk semua surface siswa baru.
- UjianPlayer/UlanganPlayer **tone surface + minimal accent** — timer warning yellow/critical rose. Hindari background loud di body soal supaya fokus.
- Admin + guru shells netral shadcn — palette `siswa.*` di-namespace lewat `.siswa-theme` wrapper di `/siswa/layout.tsx`. Jangan dipake di luar siswa pages.
