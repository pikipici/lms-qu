...
> Status: v0.11.3 — **Fase 5 ✅ CLOSED 15/15 + UX/QA pass `22d2095`** + **Fase 6 plan ✅ DECOMPOSED 15 task locked #83-#88 + 6.A.1 ✅ DONE `3371e30` + 6.B.1 ✅ DONE `f50e7f2` + 6.B.2 ✅ DONE `76de898`**, 2026-05-22. Fase 6 Ulangan Harian (cross-bab): foundation migration 000011 + 6 tables hijau, BankSoal CRUD per-guru pribadi (locked #84) hijau 26/26 smoke, image upload 6-slot R2 prefix `soal-bank/` hijau 40/40 smoke. Lanjut 6.B.3 bulk paste.
> Owner: User (guru) + Apis (assistant)
> Last updated: 2026-05-22 (Fase 6.B.2 ✅ DONE commit `76de898` — image upload (6-slot) + presigned URL per-guru pribadi, mirror SoalBab Task 5.B.2 pattern, smoke E2E /tmp/qa-6b2.sh 40/40 PASS. Roadmap v0.11.3.)
...
**Eksekusi berikutnya: Fase 6 plan ✅ DECOMPOSED 15 task — locked #83-#88. 6.A.1 ✅ DONE `3371e30`. 6.B.1 ✅ DONE `f50e7f2`. 6.B.2 ✅ DONE `76de898`. Lanjut 6.B.3 bulk paste.**
...
- 6.B BE BankSoal CRUD + image + bulk: **2/3 ✅** — **6.B.1 ✅ DONE commit `f50e7f2`** CRUD + tag mapel/tingkat/topik handler+service+repo (mirror 5.B.1 commit `928401b`), smoke E2E 26/26 PASS (create/list/filter/get/patch version-bump/version-conflict 409/siswa 403/guru2 cross-guru 403/jawaban_invalid 400/soft delete + GET 404 + list excludes). **6.B.2 ✅ DONE commit `76de898`** image upload 6-slot R2 prefix `soal-bank/` + resize 1920px JPEG q85 + WebP→JPEG fallback + presigned URL inline 15m + atomic-swap old-key R2 cleanup + version orthogonal (mirror 5.B.2 commit `57eb504`); storage.CategoryBankSoal added (distinct dari `soal/` Fase 5). Smoke E2E /tmp/qa-6b2.sh 40/40 PASS — jpg+png+webp upload, oversize 13MB 413, non-image 415, no/invalid slot 400, replace pertanyaan + R2 old-key cleanup + replaced=true, image-url presign matching object_key, empty slot 404, delete + verify cleared, delete empty slot 404, cross-guru guru2 upload+image-url 403, siswa upload 403, version stays=1 setelah 4 swap. Sisa: 6.B.3 bulk paste pipe-delimited + tag default + 8 reason codes (mirror 5.B.3 commit `dabbdf1`)
