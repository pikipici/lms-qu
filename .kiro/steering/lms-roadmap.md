...
> Status: v0.11.14 — **Fase 5 ✅ CLOSED 15/15 + UX/QA pass `22d2095`** + **Fase 6 ✅ CLOSED 15/15** locked #83-#88 (commits `3371e30`+`f50e7f2`+`76de898`+`ceaf86b`+`ede3194`+`7d465bf`+`d2ecef9`+`205be54`+`0df6f89`+`8f77dbc`+`1269846`+`446f187`+`d9012b1`+`19060d0`), 2026-05-22. Fase 6 progress 15/15 = 100%. BE 100% + FE Guru 100% + FE Siswa 100%. 6.G.2 wired UjianPlayer (timer countdown + autosave 600ms + auto-submit on expire + image presign refresh 12m) + UjianReview (post-submit pembahasan dengan jawaban_benar emerald + jawaban_salah rose) + UjianSection orchestrator state machine (lobby ↔ playing → result → review). Lanjut Fase 7 Rekap Nilai + Activity Feed.
> Owner: User (guru) + Apis (assistant)
> Last updated: 2026-05-22 (🎉 Fase 6 ✅ CLOSED 15/15 — 6.G.2 ✅ DONE commit `19060d0` UjianPlayer + UjianReview + UjianSection state machine, smoke /tmp/qa-6g2.sh 29/29 PASS, Next build hijau, /siswa/kelas/detail/ujian 13.5 kB / 142 kB First Load. Roadmap v0.11.14.)
...
- 6.G FE Siswa: **2/2 ✅ CLOSED** — **6.G.1 ✅ DONE commit `d9012b1`** `/siswa/ujian` top-level cross-kelas lobby (mirror /siswa/tugas pattern) + `UjianLobbyCard` (discriminated window state badge + source mode badge + 4-tile grid + countdown 1s + review policy note + last attempt line + PrimaryCTA dispatch inflight=Lanjutkan / mendatang=disabled / berakhir=disabled / aktif=Mulai/Mulai-baru) + filter chips status + dropdown kelas + sort + counters tile 4-up. BE FIX siswaGroup.Get("/kelas/:id/ujian") reuse ujianHandler.ListByKelas + service-level role-branch siswa published-only. Smoke 18/18 PASS. **6.G.2 ✅ DONE commit `19060d0`** UjianPlayer + UjianReview + UjianSection orchestrator state machine (mirror 5.G.2 `6c10d19`). UjianPlayer single Card: timer countdown sticky di header (warn<5m amber, critical<1m rose) auto-submit on ≤0 guarded ref + autosave debounced 600ms (terminal codes 4xx NOT retry, transient 5xx retry 2x exp backoff) + NO immediate feedback per locked #76 + R2 image refresh 12m karena presign 15m vs durasi up to 360m + optimistic queryClient.setQueryData + submit confirm dialog. UjianReview: summary 4-cell + per-soal highlight emerald=jawaban_benar/rose=wronglyPicked + soft-deleted soal placeholder. UjianSection state machine 4-state: lobby ↔ playing → result → review (lobby = ujian metadata + 4-tile + window range Asia/Jakarta + countdown 1s + history row inflight=Resume CTA, selesai=Review CTA gated; playing = UjianPlayer onDone→result onAbort→lobby; result = post-submit summary + Lihat pembahasan gated #81 canReviewResult cek izinkan_review+dapat_review_at; review = UjianReview onBack→lobby). FE shape fixes siswa-ujian-api.ts: ItemsResult flat (no envelope, BE c.JSON(res)), StartResult add soal_ids[]+drop status, SubmitResult drop ujian_id+waktu_buka_review add dapat_review_at, BE wraps submit {"summary":...} not {"hasil":...}, ReviewItem fix poin_dapat required+add poin_maksimal+drop images (BE retroactive anti-cheat). Page route /siswa/kelas/detail/ujian?id=K&uid=U query-string mirror Bab pattern static export. Page size /siswa/ujian 10.5→11.3 kB First Load 124→125 kB; /siswa/kelas/detail/ujian NEW 13.5 kB / 142 kB First Load. Smoke /tmp/qa-6g2.sh 29/29 PASS (login + setup + start hasil_id+total+soal_ids+durasi_detik+resume=false fresh + idempotent start resume=true same hasil_id + items flat shape strip jawaban_benar anti-cheat + answer {ok:true} no is_benar + submit {summary:...} jawaban_total/benar/nilai/izinkan_review/already_submitted=false + idempotent submit already_submitted=true same nilai + review {review:...} jawaban_benar exposed post-submit + poin_dapat+poin_maksimal+urutan present + hasil aggregate refresh post-submit). 🎉 **Fase 6 CLOSED 15/15**.
