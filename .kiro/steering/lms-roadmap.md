...
> Status: v0.11.6 — **Fase 5 ✅ CLOSED 15/15 + UX/QA pass `22d2095`** + **Fase 6 plan ✅ DECOMPOSED 15 task locked #83-#88 + 6.A.1 ✅ DONE `3371e30` + 6.B 3/3 ✅ CLOSED + 6.C 2/2 ✅ CLOSED + 6.D.1 ✅ DONE**, 2026-05-22. Fase 6 Ulangan Harian (cross-bab): foundation migration 000011 + 6 tables hijau, BankSoal sub-fase ✅ CLOSED — CRUD per-guru pribadi (locked #84) hijau 26/26 smoke + image upload 6-slot R2 prefix `soal-bank/` hijau 40/40 smoke + bulk paste pipe-delimited hijau 27/27 smoke. Ujian setup sub-fase ✅ CLOSED — CRUD/duplicate/source-dispatch (locked #85 manual/random + #67 deep-copy junction) hijau 52/52 smoke. **6.D.1 Ujian start (deterministic seed locked #86) + Items (anti-cheat) hijau 28/28 smoke.** Lanjut 6.D.2 answer save delayed grade.
> Owner: User (guru) + Apis (assistant)
> Last updated: 2026-05-22 (Fase 6.D.1 ✅ DONE commit `7d465bf` — Ujian start deterministic seed + Items anti-cheat strip, smoke E2E /tmp/qa-6d1.sh 28/28 PASS. Roadmap v0.11.6.)
...
**Eksekusi berikutnya: Fase 6 plan ✅ DECOMPOSED 15 task — locked #83-#88. 6.A.1 ✅ DONE `3371e30`. 6.B 3/3 ✅ CLOSED (`f50e7f2`+`76de898`+`ceaf86b`). 6.C 2/2 ✅ CLOSED (`ede3194`). 6.D.1 ✅ DONE (`7d465bf`). Lanjut 6.D.2 answer save delayed grade.**
...
- 6.D BE Ujian flow + cron: 1/4 — **6.D.1 ✅ commit `7d465bf`** Ujian start deterministic seed locked #86 (mirror 5.D.1 commits `0346609`+`32f63ae`+`d822d46`) + Items live attempt anti-cheat strip (jawaban_benar + is_benar dropped). Pool source dispatch (locked #85): manual → ujian_soal junction; random → BankSoal ListIDsByOwnerFilter + shuffle take JumlahSoal. Single-flight via `pg_advisory_xact_lock(sha256(ujian||siswa))`. Single-attempt enforcement via partial-unique `(ujian_id, siswa_id) WHERE deleted_at IS NULL` — Ujian default tidak punya remedial chain (guru bikin Ujian baru atau cancel attempt via 6.E). Schedule guard (WaktuMulai/WaktuSelesai). EventUjian + audit. Image slots presign TTL 15m via BankSoal R2 prefix `soal-bank/`. Smoke E2E /tmp/qa-6d1.sh **28/28 PASS** (draft → ujian_not_published, no_source → 409 ujian_source_missing, manual+random start happy 201, resume idempotent 200 same hasil_id+soal_ids, items anti-cheat hadir + jawaban/is_benar tidak leak, items cross-siswa 403 hasil_not_owned, siswa2 enrolled start beda hasil). 6.D.2 answer save delayed grade (mirror 5.D.2 commit `5067f0a`), 6.D.3 submit + auto-grade tx + advisory lock locked #87 (mirror 5.D.3 commit `d262ea3`), 6.D.4 timer cron 30s reuse goroutine SoalBab — extend `timer_cron.go` second sweep block (mirror 5.D.4 commit `2587526`)
