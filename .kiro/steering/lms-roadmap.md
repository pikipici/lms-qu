...
> Status: v0.11.13 — **Fase 5 ✅ CLOSED 15/15 + UX/QA pass `22d2095`** + **Fase 6 plan ✅ DECOMPOSED 15 task locked #83-#88 + 6.A.1 ✅ DONE `3371e30` + 6.B 3/3 ✅ CLOSED + 6.C 2/2 ✅ CLOSED + 6.D 4/4 ✅ CLOSED + 6.E 1/1 ✅ CLOSED + 6.F 2/2 ✅ CLOSED `1269846`+`446f187` + 6.G.1 ✅ DONE `d9012b1`**, 2026-05-22. Fase 6 progress 14/15 = 93%. BE 100% + FE Guru 100% + FE Siswa 1/2. 6.G.1 wired siswa lobby cross-kelas /siswa/ujian dengan UjianLobbyCard discriminated state (mendatang/aktif/berakhir) + countdown 1s + attempt aggregate per ujian + BE siswaGroup route /siswa/kelas/:id/ujian (service-level role-branch published-only). Lanjut 6.G.2 FE Siswa player+review untuk close Fase 6.
> Owner: User (guru) + Apis (assistant)
> Last updated: 2026-05-22 (Fase 6.G.1 ✅ DONE commit `d9012b1` — /siswa/ujian lobby cross-kelas + UjianLobbyCard countdown + window state badge + attempt aggregate + BE siswa-side route, smoke /tmp/qa-6g1.sh 18/18 PASS, Next build hijau. Roadmap v0.11.13.)
...
**Task 5.F.2 — FE Guru: Setting + Preview + Rekap Hasil + Reset Attempt** ✅ DONE 2026-05-22 commit `4195efa`
- Files baru:
  - `frontend/lib/soalbab-setting-api.ts` — typed client GET/PUT `/bab/:id/ulangan-setting` + `friendlySettingError` mapper covering jumlah_soal_exceeds_pool, ulangan_pool_empty, invalid_waktu_buka_review, version_conflict, bab_archived, forbidden, not_found.
  - `frontend/lib/soalbab-hasil-api.ts` — typed client GET `/bab/:id/hasil-rekap` + POST `/hasil-soal-bab/:id/cancel` + `friendlyHasilError` mapper covering cancel_latihan, already_cancelled, forbidden, not_found, bab_archived, kelas_archived.
  - `components/soalbab/UlanganBabSettingForm.tsx` — controlled form jumlah_soal (1-200, capped to pool_size dari SettingView), durasi_menit (1-360), batas_attempt (1-10), izinkan_review_setelah_submit checkbox, waktu_buka_review datetime-local optional (TZ user-local, conversion ke RFC3339 UTC). Header info card: pool ulangan + status (Aktif vN / Belum disetel). Optimistic concurrency: 409 version_conflict auto-invalidate query supaya FE pegang versi terbaru.
  - `components/soalbab/RekapHasilTable.tsx` — TanStack Query GET hasil-rekap, header summary (total siswa + rata-rata kelas), tabel sticky kolom Siswa/Attempt/Reset/Nilai terbaik/Nilai terakhir/Status terakhir/Mulai terakhir/Aksi. Status badge bewarna (selesai emerald, expired amber, berlangsung sky, dibatalkan rose), cancelled_count badge rose dengan AlertTriangle icon kalau >0, tombol Reset attempt (rose) trigger CancelAttemptConfirmDialog. Cancel handler hanya enabled kalau hasil_terakhir_id ada dan status ∈ {selesai, expired, berlangsung}. Locked #76: dibatalkan tidak count attempt_no berikutnya.
  - `components/soalbab/SoalPreviewDialog.tsx` — render soal pov siswa: badge mode (latihan sky/ulangan amber/keduanya emerald) + poin chip + pertanyaan card (teks + image slot pertanyaan dengan presigned URL via TanStack Query) + 5 opsi A-E inline. Opsi yang match jawaban dapat ring emerald + bullet hijau + label "Jawaban benar" dengan CheckCircle2 icon. Image slot opsi auto-fetch presigned URL.
- Modified:
  - `components/soalbab/SoalBabList.tsx` — tambah action menu "Preview" (Eye icon) sebagai item pertama dropdown (sebelum Edit), wire `previewing` state + render `SoalPreviewDialog`.
  - `app/(authed)/guru/kelas/detail/bab/page.tsx` — replace single SoalBabList di tab Soal jadi 3 Card stacked: SoalBabList → UlanganBabSettingForm → RekapHasilTable. Import baru.
- TS gotcha resolved: TS narrowing dari state hook tidak propagate ke nested closure — bind ke local const (formNonNull, viewNonNull) setelah guard supaya nested validate/handleSubmit/handleReset retain narrowed type.
- Verify: typecheck clean (server) + build clean. Page `/guru/kelas/detail/bab` size 15.1 kB → 20.2 kB; First Load 266 kB → 272 kB (+6 kB untuk 3 komponen baru).
- lms-api restart healthz 200; out/ regenerated.
- Commit: `4195efa feat(fe-soalbab): UlanganBabSetting form + Rekap Hasil + Preview dialog (Task 5.F.2)`
...
**Task 5.G.2 — FE Siswa: Ulangan Bab lobby + play (timer + autosave + resume) + review** ✅ DONE 2026-05-22
- FE Siswa Ulangan flow: lobby (trimmed setting + history attempt + smart CTA), player (timer countdown deadline_at-now warn<5m critical<1m + autosave debounced 600ms + auto-submit guarded ref pas timer ≤0 + per-soal autosave inline error notice + NO immediate feedback per locked #76), review (jawaban_benar emerald highlight + jawaban_salah rose + summary nilai/benar/salah/persen).
- Files baru:
  - `frontend/lib/soalbab-ulangan-api.ts` (typed client lobby/history/start/answer/submit/review + 22-code friendlyUlanganError mapper).
  - `frontend/components/soalbab/UlanganLobby.tsx` (durasi/batas/sisa attempt InfoTile + ReviewPolicyNote + history list dengan StatusBadge + reviewable gating per row).
  - `frontend/components/soalbab/UlanganPlayer.tsx` (timer header + scroll list nomored + radio A-E + image presigned + dialog submit confirm).
  - `frontend/components/soalbab/UlanganReview.tsx` (rekap card + ReviewSoalCard dengan emerald/rose highlight).
  - `frontend/components/soalbab/UlanganSection.tsx` (orchestrator state machine: lobby <-> playing -> result -> review; result card include reviewable gating + post-submit invalidate lobby+history queries).
- Modified: `frontend/app/(authed)/siswa/kelas/detail/bab/page.tsx` — replace `<LatihanPlayer/>` di tab Soal dengan `<SoalTabContent/>` sub-segmented (Latihan / Ulangan Bab pill switcher).
- typecheck PASS, build PASS — siswa bab page **14kB → 16.7kB / First Load 192kB → 198kB**.
- Smoke 5.G.2 happy path:
  - login guru1+siswa1, create kelas+bab+publish, bulk 5 soal mode=ulangan, setting (3 soal/1 menit/2 attempt/izinkan_review).
  - siswa join, GET ulangan-setting (trimmed: durasi+batas+izinkan, no pool_size), GET hasil list (kosong).
  - POST start → hasil_id + deadline_at, GET items → 3 soal, **anti-cheat verified: items has jawaban_benar = false**.
  - POST 3× answer (a/b/a) → respons `{ok:true}` (no is_benar/jawaban_benar).
  - POST submit → `nilai=20 benar=2/3 already=false`, idempotent submit → `nilai=20 already=true`.
  - GET review → 3 items, **review has jawaban_benar = true**.
  - GET hasil list post-submit → `count=1 attempt_count=1 terbaik=20`.
- Commits: `6c10d19 feat(fe-soalbab): siswa Ulangan flow lobby + player + review (Task 5.G.2)` push origin+server, workspace synced.

---

## 🎉 Fase 5 ✅ CLOSED 15/15 = 100% (2026-05-22)

**Total scope delivered:** Soal Bab module — Latihan formative (no nilai persist + immediate feedback + re-attempt unlimited) + Ulangan Bab graded (deterministic seed pool snapshot + timer countdown + autosave + 1× attempt + nilai persist + remedial cancel + review gating).

**BE 10/10 endpoints + 1 addon (Items):** SoalBab CRUD + image upload + bulk paste, UlanganBabSetting GET/PUT, Latihan start/answer/finish, Ulangan start/answer/submit, Hasil review/list/cancel/rekap, Items live attempt (5.G.1 prep). Cron timer-expire 30s tick + advisory lock auto-grade.

**FE 5/5 surfaces:** Guru SoalBab editor + bulk paste + image manager (5.F.1), Guru UlanganBabSetting form + Rekap table + Preview dialog (5.F.2), Siswa Latihan player (5.G.1), Siswa Ulangan lobby+player+review (5.G.2). Tab Soal di /siswa/kelas/detail/bab dan /guru/kelas/detail/bab fully wired.

**Locked decisions enforced #76-#82:** sub-fase split, anti-cheat items strip jawaban_benar, deterministic seed `sha256(mulai_unix_micro‖siswa‖bab)[:8]`, cron 30s tick + grace 5s submit, advisory lock auto-grade vs user-submit mutex, review gating izinkan+waktu_buka, coverage gate 70% (deferred to Fase 8 TODO).
...
- 5.G FE Siswa latihan + ulangan + review: **2/2 ✅ DONE** (5.G.1 ✅ commits `e0fcb66`+`1716fab` Items endpoint + LatihanPlayer; 5.G.2 ✅ commit `6c10d19` UlanganLobby+Player+Review+Section + tab Soal sub-segmented Latihan/Ulangan)
...
**Eksekusi berikutnya: Fase 6 plan ✅ DECOMPOSED 15 task — locked #83-#88. 6.A.1 ✅ DONE `3371e30`. 6.B 3/3 ✅ CLOSED (`f50e7f2`+`76de898`+`ceaf86b`). 6.C 2/2 ✅ CLOSED (`ede3194`). 6.D 4/4 ✅ CLOSED (`7d465bf`+`d2ecef9`+`205be54`+`0df6f89`). 6.E 1/1 ✅ CLOSED (`8f77dbc`) — Fase 6 BE 100%. 6.F 2/2 ✅ CLOSED — 6.F.1 (`1269846`) BankSoal page + 6.F.2 (`446f187`) Ujian setup tab. 6.G.1 ✅ DONE (`d9012b1`) `/siswa/ujian` lobby cross-kelas. Coverage gate locked #88 ≥70% deferred to Fase 8 TODO. Lanjut 6.G.2 FE Siswa UjianPlayer+Review state machine (mirror 5.G.2 commit `6c10d19`).**
...
- 6.B BE BankSoal CRUD + image + bulk: **3/3 ✅ CLOSED** — **6.B.1 ✅ commit `f50e7f2`** CRUD + tag mapel/tingkat/topik handler+service+repo (mirror 5.B.1 commit `928401b`), smoke E2E 26/26 PASS. **6.B.2 ✅ commit `76de898`** image upload 6-slot R2 prefix `soal-bank/` + resize 1920px JPEG q85 + WebP→JPEG fallback + presigned URL inline 15m + atomic-swap old-key R2 cleanup + version orthogonal (mirror 5.B.2 commit `57eb504`); storage.CategoryBankSoal added. Smoke E2E 40/40 PASS. **6.B.3 ✅ commit `ceaf86b`** bulk paste pipe-delimited 8 kolom (drop `mode` dari SoalBab format, BankSoal cross-bab) + tag default per-batch + 7 reason codes + cap 200 line + escape `\|` literal + blank/`#`-comment skip (mirror 5.B.3 commit `dabbdf1`). All-or-nothing tx via CreateInBatches(50). Smoke E2E /tmp/qa-6b3.sh 27/27 PASS — happy 5-soal multi-line + tag default + poin default/parsed, 7 reason codes hit (empty_pertanyaan/invalid_columns/invalid_jawaban/invalid_poin/empty_jawaban_option/pertanyaan_too_long/opsi_too_long), escape `\|` literal, hard preconds rows_required/too_many/mapel_too_long, siswa 403, anon 401.
- 6.C BE Ujian setup: **2/2 ✅ CLOSED** — **6.C.1+6.C.2 ✅ commit `ede3194`** Ujian CRUD/duplicate (locked #67 — junction copied via SetUjianSoalIDs, R2 image keys SHARED via BankSoal pribadi guru locked #84 — TIDAK deep-copy attachment karena tidak ada per-row attachment) + source mode dispatch manual/random (locked #85, SourceConfigJSON discriminated jsonb) + PreviewSource endpoint untuk pool size confirm sebelum save + active-attempts guard (timing/source change → 409 active_attempts_block kalau ada Hasil.Status=berlangsung). Smoke E2E /tmp/qa-6c.sh **52/52 PASS** (CRUD happy/error, version conflict, preview manual/random/empty pool, patch source switch, cross-guru soal_not_in_bank, duplicate auto-suffix Salinan + status reset draft, siswa role branching draft 404 / published 200, anonymous 401, guru2 cross-kelas 403, delete + version conflict + missing version).
- 6.D BE Ujian flow + cron: **4/4 ✅ CLOSED** — **6.D.1 ✅ commit `7d465bf`** Ujian start deterministic seed locked #86 (mirror 5.D.1 commits `0346609`+`32f63ae`+`d822d46`) + Items live attempt anti-cheat strip (jawaban_benar + is_benar dropped). Pool source dispatch (locked #85): manual → ujian_soal junction; random → BankSoal ListIDsByOwnerFilter + shuffle take JumlahSoal. Single-flight via `pg_advisory_xact_lock(sha256(ujian||siswa))`. Single-attempt enforcement via partial-unique `(ujian_id, siswa_id) WHERE deleted_at IS NULL` — Ujian default tidak punya remedial chain (guru bikin Ujian baru atau cancel attempt via 6.E). Schedule guard (WaktuMulai/WaktuSelesai). EventUjian + audit. Image slots presign TTL 15m via BankSoal R2 prefix `soal-bank/`. Smoke E2E /tmp/qa-6d1.sh **28/28 PASS**. **6.D.2 ✅ commit `d2ecef9`** Ujian answer save delayed grade locked #76 mirror (mirror 5.D.2 commit `5067f0a`). UPSERT JawabanUjian via `repo.UpsertJawaban` ON CONFLICT (hasil_id, soal_id) DO UPDATE; IsBenar=NULL + PoinDapat=0 sampai submit/cron auto-grade locked #87. Anti-cheat: soal_id MUST in hasil.SoalIDsJSON snapshot (frozen at start). Timer guard now() ≤ deadline_at; expired → 410 ujian_timer_expired. Status guard berlangsung only; selesai/dibatalkan → 410 hasil_not_active. Ownership → 403 hasil_not_owned. Sentinels: ErrSoalNotInPool 400, ErrInvalidInput 400. EventUjian(action=answer_save) audit. Smoke E2E /tmp/qa-6d2.sh **32/32 PASS**. **6.D.3 ✅ commit `205be54`** Ujian submit + auto-grade tx + advisory lock locked #87 (mirror 5.D.3 commit `d262ea3`). tx + `pg_advisory_xact_lock(sha256("hasil-submit:"||hasil_id)[:8])` — race-safe vs cron auto-grade (6.D.4 reuse same key). Idempotent: status=selesai+nilai_total set → return existing rekap dgn already_submitted=true (no relock). Late-submit grace 5s past deadline → ErrUjianSubmitAfterGrace 410. Cancelled → 409 hasil_cancelled. Re-grade pakai banksoal.Jawaban as source of truth: is_benar = (jawaban == soal.jawaban), poin_dapat = is_benar ? soal.poin : 0; missing/empty/soft-deleted → defensive false/0. UPDATE HasilUjian (status='selesai', selesai_at, nilai_total, jawaban_benar_count, jawaban_total) + Append EventUjian(action='submit') in-tx. Audit post-commit best-effort. Response: {summary: {hasil_id, nilai_total, jawaban_benar_count, jawaban_total, selesai_at, dapat_review_at?, izinkan_review, already_submitted}}. Smoke E2E /tmp/qa-6d3.sh **32/32 PASS** (happy 4/4 nilai=20 + DB lifecycle row + jawaban graded + audit + idempotent + partial 2/4=10 + all wrong 0/4=0 + skip 1/4=3 + cross-siswa 403 + 404 + 401 + items-after-submit 410 hasil_not_active + concurrent submits 2 parallel both same nilai_total winner+loser already_submitted pattern + audit single-event). **6.D.4 ✅ commit `0df6f89`** Ujian timer expire cron 30s + auto-grade tx + shared advisory lock locked #87 (mirror 5.D.4 commit `2587526`). NEW `backend/internal/ujian/timer_cron.go` (313 lines) — separate goroutine `go ujianTimerCron.Run(rootCtx)` mirror SoalBab pattern (cross-package coupling avoided). Tick 30s + initial sweep on boot (caught 9 backlog rows from 6.D.3 smoke). Repo.ScanExpiredHasilIDs: `WHERE status='berlangsung' AND deadline_at<=now() AND deleted_at IS NULL ORDER BY deadline_at LIMIT 100`. Per-row tx: `pg_advisory_xact_lock(submitLockKey(id))` shared with FlowService.Submit (sha256 `hasil-submit:`||hasil_id) → mutually exclusive on same row. Idempotent skip kalau status≠berlangsung post-lock. selesai_at = deadline_at (NOT now() — locked policy: cron records actual timer expiry truth). Action discrimination: cron='auto_grade', submit='submit' di event_ujian audit. Refactor: extract `gradeAttemptInTx` shared helper (timer_cron.go) shared between Submit (6.D.3) dan TimerCron (6.D.4) supaya logika grading konsisten. Smoke E2E /tmp/qa-6d4.sh **20/20 PASS** (happy cron-auto-grade 4/4 nilai=20 + selesai_at=deadline_at exact + event auto_grade trigger=cron reason=timer_expired + 4 jawaban graded + empty 0 jawaban → nilai=0/4 + submit-beats-cron-race → submit win zero auto_grade events selesai_at≠deadline_at + cancelled status='dibatalkan'+deleted_at skipped + idempotent already-finalized no extra event no nilai change).
- 6.E BE Hasil consolidated: **1/1 ✅ CLOSED** — **6.E.1 ✅ commit `8f77dbc`** Hasil + Review + Cancel + Rekap consolidated (mirror 5.E.1 commit `8c55651`). 4 endpoints: GET `/siswa/hasil-ujian/:id/review` (two-sentinel gating: ErrReviewDisabled 403 review_disabled vs ErrReviewLocked 403 review_locked, locked #81 enforced via Ujian.IzinkanReviewSetelahSubmit + WaktuBukaReview embedded), GET `/siswa/kelas/:id/ujian/hasil` (cross-ujian list per kelas via `ListHasilBySiswaAllKelas` JOIN ujian.kelas_id, aggregate nilai_terbaik+terakhir+attempt_count single-pass), POST `/hasil-ujian/:id/cancel` (guru/admin RoleGuard via flat hasilUjianGuruGroup, soft-cancel Status='dibatalkan'+DeletedAt set untuk release partial-unique slot supaya siswa boleh start fresh, idempotent fast-path return existing snapshot bukan 409, EventUjian action='cancelled' + audit prev_status forensics), GET `/ujian/:id/hasil-rekap` (per-siswa aggregate three-structure pattern map+slice+SiswaRekap, sort *float64 DESC nulls-last + name ASC tiebreak, rata-rata avg(nilai_terbaik) over students-with-score not total, narrow userLookup interface authRepo nil-safe degrade). Pool-snapshot delete-tolerant render (placeholder kalau soal dihapus). Pattern reference skill `go-attempt-readside-review-cancel-rekap`. Smoke E2E /tmp/qa-6e1.sh **37/37 PASS** (T1 happy review 4/4 + jawaban_benar exposed, T2 cross-siswa 403 forbidden, T3 berlangsung 409 hasil_not_finished, T4 izinkan_review=false 403 review_disabled distinct code, T5 waktu_buka_review future 403 review_locked distinct code, T6 siswa list per kelas attempt_count/nilai_terbaik, T7 cancel berlangsung dibatalkan+deleted_at+event, T8 cancel idempotent same snapshot no double event, T9 siswa POST cancel 403 RoleGuard, T10 post-cancel siswa start fresh slot-released, T11 rekap 2 siswa scores total=2 rata_rata=10 sort first=20, T12 nulls-last sort: nilai-siswa first null-siswa second, T13 siswa GET rekap 403 RoleGuard, T14 invalid uuid 400 invalid_id). Coverage gate locked #88 ≥70% — soft fallback DEFER ke Fase 8 TODO (mirror Fase 5 #82). Smoke E2E hijau cukup untuk MVP.
- 6.F FE Guru: **2/2 ✅ CLOSED** — **6.F.1 ✅ DONE commit `1269846`** `/guru/bank-soal` top-level page (locked #84 BankSoal pribadi cross-kelas, bukan nested di kelas) + `BankSoalList` (filter chips Mapel/Tingkat derived dari master query + Topik substring search + pagination 25/page server limit/offset) + `BankSoalEditDialog` (tag mapel/tingkat/topik + 5 opsi + jawaban radio + poin 1-100 + 6 image slots presigned preview, drop `mode` dropdown vs SoalBab, default tag dari filter aktif untuk speed UX) + `BankSoalBulkPasteDialog` (pipe-delimited 8 kolom drop `mode`, top-level body kirim mapel/tingkat/topik default ke setiap soal, partial-success result panel) + nav entry "Bank Soal" di guru sidebar. Optimistic concurrency #56 PATCH+DELETE bawa version. Image swap NOT bump version (locked #78). Next build hijau (`/guru/bank-soal` 14.9 kB / 153 kB First Load JS, masuk static export). Smoke /tmp/qa-6f1.sh 14/14 PASS (list shape `items+total+limit+offset`, limit/offset query, create owner_guru_id auto, filter mapel exact + topik substring, PATCH version 1→2, stale version → 409 version_conflict, bulk 8-col `created+errors[]` + tag default propagated, bulk empty → rows_required, soft-delete deleted=true, post-delete hidden dari list). **6.F.2 ✅ DONE commit `446f187`** Ulangan tab di `/guru/kelas/[id]` detail page + `UjianList` (card per ujian + status badge draft/published/archived + source mode badge manual/random + jumlah_soal preview + timing range + Review gating display + dropdown Edit/Duplikat/Hapus + chevron expand inline rekap) + `UjianFormDialog` (judul + deskripsi + durasi 1-360 + waktu_mulai/selesai datetime-local + izinkan_review_setelah_submit toggle + waktu_buka_review optional + status draft/published + version-bound PATCH 409 invalidate) + `UjianSourceConfigPanel` discriminated source selector locked #85 (mode manual: filter chip mapel/tingkat + topik substring search + checkbox grid dari BankSoal pribadi cap 200 + count terpilih + Pilih semua terlihat / Kosongkan; mode random: dropdown chip dari mapel/tingkat tag + topik input + jumlah_soal 1-200; Live Preview button POST /ujian/:id/source/preview untuk verify pool_size sebelum save, hint create-mode preview butuh ujian id; pool warning state pool=0 destructive pool<jumlah amber) + `RekapHasilUjianTable` per-siswa aggregate (attempt_count + cancelled_count badge + nilai_terbaik/terakhir + status_terakhir berwarna + tombol Reset attempt) + `CancelUjianAttemptDialog` confirm soft-cancel POST /hasil-ujian/:id/cancel (locked #76 dibatalkan tidak count attempt_no, slot released untuk fresh attempt). frontend/lib/ujian-api.ts typed client + friendlyUjianError covering invalid_source/source_pool_empty/soal_not_in_bank/version_conflict/attempts_exist/active_attempts_block/review_locked/review_disabled/hasil_already_cancelled. Tab nav guru kelas detail ditambah entry "Ulangan" (GraduationCap icon). Page size guru/kelas/detail 6.94 kB → 16.7 kB (+9.76 kB), First Load 269 kB, masuk static export. Smoke /tmp/qa-6f2.sh 14/14 PASS (list shape, create draft, create 3 BankSoal, PATCH source manual v1→2, preview manual pool=3 jumlah=3, PATCH source random v→3 mode=random, preview random pool=3 filter mapel, stale version → 409 version_conflict, invalid source mode → invalid_source, rekap empty total=0, duplicate → draft + Salinan judul, delete deleted=true).
- 6.G FE Siswa: 1/2 — **6.G.1 ✅ DONE commit `d9012b1`** `/siswa/ujian` top-level cross-kelas lobby (mirror /siswa/tugas pattern) + `UjianLobbyCard` (discriminated window state badge mendatang/aktif/berakhir/tersedia + source mode badge manual/random + 4-tile grid durasi/jumlah_soal/attempt_count/nilai_terbaik + window range Asia/Jakarta + countdown auto-tick 1s ke waktu_mulai/waktu_selesai + review policy note locked #81 + last attempt line dengan reviewable gating + PrimaryCTA dispatch inflight=Lanjutkan / mendatang=disabled "Mulai dalam ..." / berakhir=disabled "Window berakhir" / aktif=Mulai/Mulai-baru) + filter chips status (semua/aktif/mendatang/berakhir) + dropdown kelas + sort aktif-deadline-first / mendatang-soonest-first / berakhir-most-recent + counters tile 4-up. CTA navigate ke /siswa/kelas/detail/ujian?id=K&uid=U deliberate two-stage rollout (route 404 sampai 6.G.2 ship). frontend/lib/siswa-ujian-api.ts typed client siswa-side (listSiswaUjianByKelas + listSiswaUjianHasil + 6.G.2 prep start/items/answer/submit/review) + friendlySiswaUjianError 16-code mapper. BE siswaGroup.Get("/kelas/:id/ujian") di main.go reuse ujianHandler.ListByKelas — service-level role-branch siswa auto-filter status=published only + verify enrollment (sebelumnya endpoint cuma admin/guru, siswa kena 403). Page size /siswa/ujian 10.5 kB / First Load JS 124 kB (static export). Sidebar nav siswa ditambah entry "Ujian saya" GraduationCap icon. Smoke /tmp/qa-6g1.sh 18/18 PASS (login guru1+siswa1, 3 banksoal create, ujian draft create, source manual 3 soal, publish, ujian draft hidden, siswa list non-empty, hidden tidak terlihat, published terlihat, source_config_json shape, durasi+review_flag, hasil aggregate shape kelas_id+items+attempt_count, enrollments hydration, kelas.nama present, archive ujian → siswa hilang). 6.G.2 sisa — UjianPlayer (timer countdown header warn<5m critical<1m + autosave debounced + auto-submit on timer expire guarded + per-soal inline autosave error notice + NO immediate feedback locked #76) + UjianReview (jawaban_benar emerald + jawaban_salah rose + summary card) + UjianSection orchestrator state machine (lobby ↔ playing → result → review) (mirror 5.G.2 commit `6c10d19`).

**Konsep beda dari Fase 5:**
- Bank Soal = guru-owned pool (per-guru pribadi locked #84), tidak bab-tied. Soal lintas-bab.
- Ujian instance per-kelas, source manual/random dari Bank Soal owner=guru kelas (locked #85).
- HasilUjian: 1 attempt per (Ujian, Siswa) dengan partial unique `WHERE deleted_at IS NULL`. Reset = remedial soft-delete + audit (mirror locked #45).
- Review gating mirror #81: boolean `IzinkanReviewSetelahSubmit` + nullable `WaktuBukaReview`.
